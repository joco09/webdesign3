...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    $userQuery = "SELECT * FROM User_member_table WHERE E_mail = ?;";
...
//            setcookie($cookiename ="usercookie",$cookievalue = $userResultArray['First_name'], time() + 86400);
//            if(!isset($_COOKIE[$cookiename])) {
//                echo "<h1>Cookie named '" . $cookiename . "' is not set!</h1>";
//            } else {
...
//            }
    ...
    {
        //prepared statement for sql injection prevention
        $staffQuery = "SELECT * FROM User_staff_table WHERE E_mail = ?;";
        $stmt= mysqli_stmt_init($connection);

        if(!mysqli_stmt_prepare($stmt,$staffQuery)){
            echo "SQL statement failed";
        }else{
            // binding params to prepared statement
            mysqli_stmt_bind_param($stmt, "s", $userEmail);
        ...
        if (mysqli_num_rows($staffResult) > 0)
        {
            ...
            if(password_verify($userPassword,$staffResultArray['Password']))
            {
                session_start();
