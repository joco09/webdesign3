...
if ((isset($_COOKIE["rememberMe"])) && (empty($_SESSION['login'])))  { // If we have a value in the rememberMe cookie and they aren't already logged in
    ...
    if ((!empty($cookieData["authenticationToken"])) && ($cookieData["authenticationToken"] == "64jkf0wjdtqbausjsks")) { // If cookie value is correct
        ...
        if ($result) {
            ...
            if ($found > 0) { // If we have at least 1 row returned we know the user exists
                ...
                /* Work out where to send the user. If the user tried to access a protected page (i.e. one for 
                * logged in users only) but weren't logged in they'll be sent to this page to log in. Therefore 
                * we see if we have a HTTP_REFERER value in the PHP server variables, which tells us the 
                * previous page the user went to before being sent to this page, so that we can redirect them 
                * back there now they are logged in. However, if there isn't a HTTP_REFERER value, e.g. they 
                * went direct to this page, or if the referer value is a page we don't want to send them back 
                * to (such as / or index.php which is the page they are currently on so redirecting them back to 
                * the same page would create an endless loop), or if the previous page was logout.php so we 
                * wouldn't want to redirect them back there as they don't need to log out again) then send them 
                * to the homepage instead. 
                * 
                * Note the use of ternary operators here - see 
                * https://www.php.net/ternary#language.operators.comparison.ternary for an explanation of them
                */    
               $referrer = (
...
<html>
    <head>
        <title>Foobar - A social network you can trust</title>    
    ...
    <body class="no-sidebars" id="top">         
        <header id="mainH">
            <div class="inner-cont">
                <h1>Foobar - A social network you can trust</h1>
                <a href="#" title="Home" id="homeLink">                    
                    <span class="hgroup">
                        <h2>Created by</h2>
                        <h3>Paul Albinson FdSc BSc (Hons) PGCEP MSc MBCS FHEA</h3>
                    </span>
                </a>                      
            </div>
        </header>
        <div id="container">
            <main role="main">
                <h1>Welcome to Foobar - A social network you can trust</h1>
<?php
if (mysqli_connect_error()) { // If connection error
    ...
    if ((!empty($_SESSION['login'])) && ($_SESSION['login'] == true)) { // If user is already logged in
        ...
        ?>
                <h1>Use the form below to log in</h1>
...
?>
        ...
        <footer id="mainF">
            <div role="contentinfo">
                <p>Copyright Paul Albinson 2014 - <?php echo Date("Y"); ?> &copy;</p>
                <ul class="inline">
                    <li>
